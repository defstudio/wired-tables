...
@if($this->shouldShowFiltersSelector())
    ...
    @if($this->config(\DefStudio\WiredTables\Enums\Config::group_filters))
        <div {{$attributes->class('relative')}} wire:key="wt-{{$this->getId()}}-filters-wrapper" x-data="{show: false}">
            <button
                wire:key="wt-{{$this->getId()}}-filters-dropdown"
                {{$attributes->class("flex items-center px-2 py-2 bg-transparent border focus-visible:outline-0 border-solid border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm text-sm text-gray-700 ")}}
                @click="show = !show"
            >
                Filters
            ...
            <div wire:key="wt-{{$this->getId()}}-filters-container"
                 x-show="show"
                ...
                @foreach($visibleFilters->chunk($this->config(\DefStudio\WiredTables\Enums\Config::filters_columns, $visibleFilters->count() > 2 ? 2 : 1)) as $filter_group)
                    <div class="table-row">
                        @foreach($filter_group as $filter)
                            ...
                            <div wire:key="wt-{{$this->getId()}}-filter-{{$filter->key()}}-container" class="table-cell p-2">
    ...
    @else
        <div {{$attributes->class('flex')}} wire:key="wt-{{$this->getId()}}-filters-wrapper">
